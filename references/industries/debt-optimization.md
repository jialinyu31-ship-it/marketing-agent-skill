...
Use this file when the user's niche, product, client, or content request matches `债务优化`. This is a synthesized playbook derived from the C-column Agent row `49: 债务优化行业观点情绪选题生成器` and its set attachment name `债务优化行业观点情绪选题生成器.md`. Do not copy the original source prompt text.
