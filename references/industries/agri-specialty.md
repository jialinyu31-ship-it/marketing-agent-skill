...
Use this file when the user's niche, product, client, or content request matches `三农特产`. This is a synthesized playbook derived from the C-column Agent row `37: 三农特产行业观点情绪选题生成器` and its set attachment name `三农特产行业观点情绪选题生成器.md`. Do not copy the original source prompt text.
